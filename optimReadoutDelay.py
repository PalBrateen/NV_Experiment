...
N_scanPts = 50  # Number of delay steps

# Pulse sequence parameters:----------------------------------------------------
t_AOM = 5*us  # AOM pulse duration (in ns)
...
saveFileName = "optimizeReadoutDelay_"  # File name for data file
...
try:
    ...
    #PB channels
    PBchannels = {'DAQ':DAQ,'STARTtrig':STARTtrig,'AOM':AOM}
    #Make save file path
    ...
    #%% Pulse sequence plot
    # imgpath = os.getcwd()+"\\OptReadout_fig\\"
    # imageFileName = "OptReadout_"
    ...
    if plotPulseSequence:
        j=1
        for i in [0,-1]:
            instructionList = PBctl.PB_program('optimReadoutSeq', [t_readoutDelay[i],t_AOM])
            ...
            plt.ylabel('channel')
            plot_label = '1st' if j==1 else 'Last'
            plt.title('Pulse Sequence at '+plot_label+' scan pt')
            j+=1
        # if savefig == 
        # fig.savefig(imgpath+imageFileName+'.jpg',dpi=150,bbox_inches='tight',quality=85)
        # plt.close(fig)
        # plt.close('all')
        plt.xlabel('time (us)')
    #%% Run Sequence
    ...
    for i in range (0, N_scanPts):
        instructionList= PBctl.PB_program('optimReadoutSeq', [t_readoutDelay[i],t_AOM])  # PB
