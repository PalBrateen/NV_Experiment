...
import time
import cv2, numpy as np, tifffile as tifff
...
def show_live_captured_images(dcamcon: Dcamcon):
    ...
    # t_exposure = dcamcon.setget_propertyvalue(DCAM_IDPROP.EXPOSURETIME, val=0.05)
    number_of_frames = 10
    # prepare buffer
    ...
    dcamcon.releasebuffer()
    return wait_event_time, all_frames
...
if __name__ == '__main__':
    ...
    if dcamcon_init():
        ...
        if dcamcon is not None:
            ...
            if (not signaled_sigint and
                res):
                [wait_event_time, all_frames] = show_live_captured_images(dcamcon)
    ...
    if save_at_finish_live:
        ...
        tifff.imwrite('frames.tiff', all_frames, imagej=True)
