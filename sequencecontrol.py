...
import sys, math, numpy as np, matplotlib.pyplot as plt, control_camera_sequences as cam_seq,\
    control_daq_sequences as daq_seq, PBcontrol as PBctrl
...
class sequencecontrol():
    ...
    def check_params(self, config_file):
        self.pb = PBctrl.PulseBlaster()
        self.clk_cyc = self.pb.clk_cyc
        ...
        if expCfg.sequence in ['esr_seq', 'pesr_seq', 'ram_seq', 'T2_seq', 'modesr']:
            if expCfg.t_AOM%(2*self.clk_cyc):
                print('\x10 Warning: t_duration set to ', expCfg.t_AOM,'ns, which is not an integer multiple of ',(2*self.clk_cyc),'ns. Rounding t_duration to nearest multiple of ',(2*self.clk_cyc),'ns...')
        ...
        if expCfg.sequence in ['rabi_seq', 'T1seq', 'T1ms0', 'rodelay', 'ram_seq', 'T2_seq'] or (expCfg.sequence == 'T2seq' and expCfg.numberOfPiPulses == 1):   # why to check for numberOfPiPulses==1??
            if step_size<self.clk_cyc:
                ...
                # print('\t\x10 Step Size = '+str(step_size)+" > "+str(self.clk_cyc))
                None
    ...
    #-----------------------------------------------------------------------
    def plot_data(self, x,y,xlabel,ylabel,formatting,yTicks,yTickLabels, title):
    ...
    def plot_sequence(self, instructions, channelMasks):
        """
    ...
    def sequence_event_cataloguer(self, allPBchannels):
        """
    ...
    def param_err_check(self, instr, sequence, PBchannels, seqArgList, parameter=[0,1], Nscanpts=1):
        # Trial run to check whether the durations of all Inst < 10ns (=5*self.clk_cyc)
        ...
        for i_scanpt in range (0, Nscanpts):     # scan over all the scannedParam values
            ...
            the_list = self.pb.PB_program(instr, sequence, seqArgList, err_check=True)
    ...
    def make_sequence(self, instr, sequence, args):
