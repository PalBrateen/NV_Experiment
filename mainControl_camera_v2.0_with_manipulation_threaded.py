...
global expCfgFile, trial_run, seq_no_plot, voltage_unit, seq_plot_dpi, plotPulseSequence, clk_cyc, SG, expCfg, filenumber, hdcamcon, instr
...
def initialize_exp(instr, expCfg):
    ...
    return [roi, savePath, param_save_format, seqArgList, expParamList, Nscanpts, param, instructionList] if trial_run[1] == 'n' else [
...
def extra_param_save_details(scan_time_list, exec_time, roi):
...
def save_data(datafilename, data_raw):
...
def acquire_data(trigger_event, condition, roi, Nsamples, Nscanpts):
        ...
        for i_scanpt in range(0, Nscanpts):
            ...
            for i_sample in range(0, Nsamples):
                ...
                if frame is not False:
                    ...
                    data_raw[i_scanpt,i_sample,:,:] = frame
                else:
                    print(" No frame...")
                
                if not (i_sample+1 == Nsamples):    # execute if not the last sample
                    result = hdcamcon.wait_capevent_frameready(timeout_ms)
            
            scan_time_list.append(time.perf_counter() - scan_start_time)
            print('Single scan point end...')

            # stop PB and clear the event, notify all waiting threads...
    ...
    return [data_raw, scan_time_list, i_scanpt]
...
def pulse_sequence_loop(*timings, t_seq_total, parameters, sequence, seqArgList, trigger_event, condition):
    """ The loop to control the scanpoint loop and pulse sequence for experiment as well as camera trigger
    ...
    # for i_run in range(0, Nruns):     # TODO: add Nruns loop as well
    for i_scanpt in range(0, Nscanpts):
        ...
        with condition:
            ...
            while trigger_event.is_set():   # this statement dictates the wait condition: wait until the condition evaluates to False = event cleared..
            # for 1st execution, this is False
                condition.wait()        # waits for some event notification via notify() or notify_all()..
    ...
    if display_parameters == 'yes':
        if trial_run[1] == 'n':
            try:
                ...
                scan_thread = threading.Thread(target=pulse_sequence_loop, args=(t_exposure * 1e9, N_total, t_align * 1e9, t_seq_total, param, expCfg.sequence, seqArgList, trigger_event, condition))
                scan_thread.start()
                
                # Main thread handling camera acquisition
                acq_start_time = time.perf_counter()
                [data_raw, scan_time_list, i_scanpt_cam] = acquire_data(trigger_event, condition, roi, Nsamples, Nscanpts)
                ...
                if i_scanpt_inst == i_scanpt_cam:
                    i_scanpt = i_scanpt_inst
                else:
                    print("Error is acquisition: i_scanpt_inst != i_scanpt_cam!!")
                                    
                # scan complete.. process for display...
                ...
                if savefile_yn == 'yes':
                    ...
                    save_flag = save_data(datafilename, data_raw)
                else:
            ...
            #         save_data(len(conCfg.input_terminals), datafilename, data_raw, expCfg.Nsamples, reads_per_cyc)
                ...
                print("\x10 shape(frames) = \x1b[38;2;250;150;50m%d x %d x %d\x1b[0m" % np.shape(data_raw[0,:,:,:]))
                ...
                if save_flag:
                    ...
                    [next_params_format, expParamList] = extra_param_save_details(scan_time_list, exec_time, roi)
