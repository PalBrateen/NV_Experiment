...
def PB_program_camera(sequence, sequenceArgs, err_check):
    List = []
...
def PB_program_diode(sequence, sequenceArgs, err_check):
...
def run_sequence_for_camera(instructionList,t_exposure,t_seq_total):
    ...
    N_trigger = [int(np.floor(t_cam_response/element)) if element>0 else 0 for element in t_seq_total]
    N_remaining = [int(np.floor((t_exposure - t_cam_response)/element)) if element>0 else 0 for element in t_seq_total]
    N_total = [int(np.floor(t_exposure/element)) if element>0 else 0 for element in t_seq_total]
    ...
    # t_buffer = [clk_cyc*round(time/clk_cyc) for time in t_buffer]
    # t_buffer = [10.0 if time<10*ns else time for time in t_buffer]
    # print(t_buffer)
    ...
    # print('Sig: ',signal_instruction)
    reference_instruction = instructionList[1]
    # print('Ref: ',reference_instruction)
    ...
    if N_trigger[1] >= 1:
        ...
        status = pb_inst_pbonly(reference_instruction[i+1][0]^concfg.camera, Inst.END_LOOP, start[0], reference_instruction[i+1][3]) if i>0 else 0
        ...
        status = pb_inst_pbonly(0^concfg.camera, Inst.CONTINUE, 0, t_buffer[3]) if i>0 else pb_inst_pbonly(concfg.laser^concfg.camera, Inst.CONTINUE, 0, t_buffer[3])
        # print(0, Inst.CONTINUE, 0, t_buffer[3])
        errorCatcher(status)
    else:
        start[0] = pb_inst_pbonly(concfg.camera, Inst.CONTINUE, 0, t_cam_response) if not t_seq_total[1]==0.0 else pb_inst_pbonly(concfg.laser^concfg.camera, Inst.CONTINUE, 0, t_cam_response)
        errorCatcher(start[0])              # time = 't_cam_response' or 't_buffer[3]'
    #------------------------------------------------------------------------------------
    # 2nd: sequence has 'no' camera, with the sequence (instructionList[0]) running...
    started = False
    # the buffer time is at the beginning in this case...
    status = pb_inst_pbonly(0, Inst.CONTINUE, 0, t_buffer[0]);
    ...
    errorCatcher(status)
    #------------------------------------------------------------------------------------
    # 3rd: sequence has 'camera' again (XORed with concfg.camera)... with the signal sequence (instructionList[0]) running...
    if N_trigger[0] >= 1:
        started = False
        for i in range(0, len(signal_instruction)-1):
            # print(signal_instruction[i])
        ...
        status = pb_inst_pbonly(0^concfg.camera, Inst.CONTINUE, 0, t_buffer[1]);
        # print(0^concfg.camera, Inst.CONTINUE, 0, t_buffer[1])
        errorCatcher(status)
    else:
        start.append(pb_inst_pbonly(0^concfg.camera, Inst.CONTINUE, 0, t_cam_response)); errorCatcher(start[2])
    #------------------------------------------------------------------------------------
    # 4th: sequence runs w/o 'camera'... with the 'reference' sequence (instructionList[1])...
    started = False
    # buffer comes here...
    status = pb_inst_pbonly(0, Inst.CONTINUE, 0, t_buffer[2]) if not t_seq_total[1]==0.0 else pb_inst_pbonly(0^concfg.laser, Inst.CONTINUE, 0, t_buffer[2])
    ...
    status = pb_inst_pbonly(reference_instruction[i+1][0], Inst.END_LOOP, start[3], reference_instruction[i+1][3]) if i>0 else 0
    ...
    status = pb_inst_pbonly(reference_instruction[i+1][0], Inst.BRANCH, start[0], 10*ns) if i>0 else pb_inst_pbonly(0^concfg.laser, Inst.BRANCH, start[0], 10*ns)
