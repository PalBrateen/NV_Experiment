...
class PulseBlaster:
    ...
    def pb_inst_pbonly(flags, inst, inst_data, length):
        """   
        Define pb_inst_pbonly in a new way. It is already defined in the spinapi.py file.

        This method accepts inputs that are easily understandable and then converts it into the types that the function in spinapi.py file requires.
    ...
    @staticmethod
    def PB_program(instr: str, sequence: str, sequenceArgs:list, err_check:bool=False):
        """
        Program the Pulse Blaster with the sequence.
        
        Parameters
        ----------
        instr : str
            Options: cam, cam_level1, cam_levelm, diode, timeseries, etc.
        sequence : str
            Name of the sequence from config file
        sequenceArgs : list
            Timing parameters and PBchannel dictionary.
        err_check : bool, optional
            Whether to check for errors in the sequence. The default is False.
        """
        ...
        if instr == 'cam':
            return PulseBlaster.PB_program_camera(sequence, sequenceArgs, err_check)
        elif instr == 'cam_level1':
            return PulseBlaster.PB_program_camera_level_trigger_1(sequence, sequenceArgs, err_check)
        elif ('cam_levelm' in instr) or ('cam_syncm' in instr):
            return PulseBlaster.PB_program_camera_trigger_many(sequence, sequenceArgs, err_check)
        elif instr == 'diode':
            return PulseBlaster.PB_program_diode(sequence, sequenceArgs, err_check)
    ...
    def run_sequence_for_camera(self, instructionList, t_exposure, t_seq_total, N_total):
        ...
        if N_trigger[1] >= 1:
            ...
            status = pb_inst_pbonly(reference_instruction[i + 1][0] ^ concfg.camera, Inst.END_LOOP, start[0], # type: ignore
                                    reference_instruction[i + 1][3]); # type: ignore
        ...
        self.errorCatcher(status)
        #------------------------------------------------------------------------------------
        # 3rd: sequence has 'camera' again (XORed with concfg.camera)... with the signal sequence (signal_instruction) running...
        if N_trigger[0] >= 1:
            started = False
            for i in range(0, len(signal_instruction) - 1):
                # print(signal_instruction[i])
        ...
        self.pb_status = "running"
        # status = pb_close();                self.errorCatcher(status)
        # print('\x1b[38;2;250;0;0m PB STARTED\x1b[0m')
    ...
    def run_sequence_for_camera_level_trigger_many(self, instructionList, t_exposure, t_seq_total,
                                                   N_total, Nsamples):
        # with level trigger
        # t_seq_total should be a list: t_seq_total = [t_seq_tot_sig, t_seq_tot_ref]
        # print(instructionList)
        ...
        if N_total == []:
            ...
            # print(t_exposure)
            N_total = [int(np.floor((t_exposure-t_cam_response) / element)) for element in t_seq_total]
        ...
        # part 1: camera exposure start edge and subsequent delay
        # do we need laser here?
        b_channels = concfg.bx ^ concfg.by ^ concfg.bz
        pbchannels = concfg.camera ^ concfg.laser ^ b_channels#^ concfg.MW
        start.append(pb_inst_pbonly(pbchannels, Inst.LOOP, Nsamples, t_cam_response))
        self.errorCatcher(start[0])
        # print(f"\x1b[38;2;250;100;0mSignal\x1b[0m: {pbchannels}, {Inst.LOOP}, {Nsamples}, {t_cam_response}")

        # part 2: the SIGNAL sequence loop while camera exposure
        ...
        if N[0] >= 1:  # use the else case for ODMR!!!!!!!!!
            ...
            for i in range(0, len(instruction) - 1):
                ...
                if started:
                    # middle instructions...
                    pbchannels = instruction[i][0] ^ concfg.camera ^ b_channels
                    status = pb_inst_pbonly(pbchannels, instruction[i][1], instruction[i][2], instruction[i][3]);
                    self.errorCatcher(status)
                    # print(f"{pbchannels} ({instruction[i][0]}), {instruction[i][1]}, {instruction[i][2]}, {instruction[i][3]}")
                else:
                    # first instruction.. with Inst.LOOP and repetitions N_trigger(_signal)[0]
                    # start[1]
                    pbchannels = instruction[0][0] ^ concfg.camera ^ b_channels
                    start.append(pb_inst_pbonly(pbchannels, Inst.LOOP, N[0], instruction[0][3]));
                    self.errorCatcher(start[1])
                    # print(f"{pbchannels} ({instruction[0][0]}), {Inst.LOOP}, {N[0]}, {instruction[0][3]}")
                    started = True

            # the last instruction.. with Inst.END_LOOP and start point (start[2])...
            pbchannels = instruction[i + 1][0] ^ concfg.camera ^ b_channels
            status = pb_inst_pbonly(pbchannels, Inst.END_LOOP, start[1], instruction[i + 1][3]);
            self.errorCatcher(status)
            # print(f"Last: {pbchannels}, {Inst.END_LOOP}, {start[1]}, {instruction[i + 1][3]}")

            # the buffer comes here..
            pbchannels = 0 ^ concfg.camera ^ b_channels
            # status = pb_inst_pbonly(pbchannels, Inst.CONTINUE, 0, t_buffer[0]) if t_buffer[0] >= 10 else 0
            # NOT 0 for else case since the CONTINUE inst is required...
            status = pb_inst_pbonly(pbchannels, Inst.CONTINUE, 0, t_buffer[0]) if t_buffer[0] >= 10 \
                else pb_inst_pbonly(pbchannels, Inst.CONTINUE, 0, 10*ns)
            self.errorCatcher(status)
            # print(f"Buffer: {pbchannels}, {Inst.CONTINUE}, {0}, {t_buffer[0]}") if t_buffer[0] >= 10 \
                # else # print(f"Buffer: {pbchannels}, {Inst.CONTINUE}, {0}, {10*ns} ({t_buffer[0]})")

        else:  # -------------check this out for normal ODMR!!!!!!--------------
            pbchannels = concfg.laser ^ concfg.MW ^ concfg.camera ^ b_channels
            # start[1]
            ...
            self.errorCatcher(start[1])
            # print(f"{pbchannels}, {Inst.CONTINUE}, {0}, {t_exposure[0]}")

        # part 3: gap between frames
        # TODO: automate the gap between frames
        t_gap = 8*ms
        pbchannels = 0 ^ b_channels #^ concfg.laser
        status = pb_inst_pbonly(pbchannels, Inst.CONTINUE, 0, t_gap)
        # print(f"Gap: {pbchannels}, {Inst.CONTINUE}, {0}, {t_gap}")
        self.errorCatcher(status)

        # part 4: camera exposure edge and subsequent delay
        # do we need laser here? - maybe yes? for initialization!
        pbchannels = concfg.camera ^ concfg.laser ^ b_channels
        status = pb_inst_pbonly(pbchannels, Inst.CONTINUE, 0, t_cam_response);
        self.errorCatcher(status)
        # print(f"\x1b[38;2;250;100;0mReference\x1b[0m: {pbchannels}, {Inst.CONTINUE}, {0}, {t_cam_response}")

        # part 5: the REFERENCE sequence loop while camera exposure
        ...
        if N[1] >= 1:  # use the else case for ODMR!!!!!!!!!
            ...
            for i in range(0, len(instruction) - 1):
                ...
                if started:
                    ...
                    self.errorCatcher(start[2])
                    # print(f"{pbchannels} ({instruction[i][0]}), {Inst.LOOP}, {N[1]}, {instruction[0][3]}")
                    started = True
            ...
            self.errorCatcher(status)
            # print(f"Last: {pbchannels}, {Inst.END_LOOP}, {start[2]}, {instruction[i + 1][3]}")

            # the buffer comes here..
            # the last instruction with Inst.CONTINUE...
            pbchannels = 0 ^ concfg.camera ^ b_channels
            # status = pb_inst_pbonly(pbchannels, Inst.CONTINUE, start[0], t_buffer[1]) if t_buffer[1] >= 10 else 0
            # NOT 0 for else case since the CONTINUE inst is required...
            status = pb_inst_pbonly(pbchannels, Inst.CONTINUE, 0, t_buffer[1]) if t_buffer[1] >= 10 \
                else pb_inst_pbonly(pbchannels, Inst.CONTINUE, 0, 10*ns)
            self.errorCatcher(status)
            # print(f"Buffer: {pbchannels}, {Inst.CONTINUE}, {0}, {t_buffer[1]}") if t_buffer[1] >= 10 else \
                # print(f"Buffer: {pbchannels}, {Inst.CONTINUE}, {0}, {10*ns} ({t_buffer[1]})")

        else:  # check this out for normal ODMR!!!!!!
            pbchannels = concfg.laser ^ concfg.camera ^ b_channels
            # start[2]
            start.append(pb_inst_pbonly(pbchannels, Inst.CONTINUE, 0, t_exposure[1]));
            self.errorCatcher(start[2])
            # print(f"{pbchannels}, {Inst.CONTINUE}, {0}, {t_exposure[1]}")
        
        # part 6: gap between frames
        # TODO: automate the gap between frames
        pbchannels = 0 ^ b_channels #^ concfg.laser
        status = pb_inst_pbonly(pbchannels, Inst.END_LOOP, start[0], t_gap)
        # print(f"Gap: {pbchannels}, {Inst.END_LOOP}, {start[0]}, {t_gap}")
        self.errorCatcher(status)

        pbchannels = 0 ^ b_channels
        pb_inst_pbonly(pbchannels, Inst.CONTINUE, 0, 14*ns)
        # print(f"End:, {pbchannels}, {Inst.CONTINUE}, {0}, {14*ns}")
        # the above CONTINUE statements are required to set the channels to 0
        # otherwise the channels will remain in the state of the last instruction (part 6)
        pb_inst_pbonly(pbchannels, Inst.STOP, 0, 10*ns)
        # print(f"END: {pbchannels}, {Inst.STOP}, {0}, {10*ns}")
        #---------------------------------------------DONE-------------------------------------------
    ...
    def run_sequence_for_camera_sync_trigger_many(self, instructionList, t_exposure, t_align, t_seq_total, N_total, Nsamples):
        # with sync-readout trigger
        # t_seq_total should be a list: t_seq_total = [t_seq_tot_sig, t_seq_tot_ref]
        # print(instructionList)
        # instructionList               => len(instructionList)=2; PB instructions for 'signal' and 'reference' sequences
        ...
        start = []  # store the 'start' points of the sequence: 1st instruction and loop start point
        # part 1: camera exposure edge and subsequent delay
        # do we need laser here?
        pbchannels = concfg.camera ^ concfg.laser ^ b_channels_only#^ concfg.MW
        start.append(pb_inst_pbonly(pbchannels, Inst.LOOP, Nsamples, t_cam_response))
        self.errorCatcher(start[0])
        # print(f"{pbchannels}, {Inst.LOOP}, {Nsamples}, {t_cam_response}")
        ...
        if N[0] >= 1:  # use the else case for ODMR!!!!!!!!!
            ...
            for i in range(0, len(instruction) - 1):
                ...
                if started:
                    ...
                    start.append(pb_inst_pbonly(pbchannels, Inst.LOOP, N[0], instruction[0][3]));
                    self.errorCatcher(start[1])
                    # print(f"{pbchannels} ({instruction[i][0]}), {Inst.LOOP}, {N[0]}, {instruction[0][3]}")
                    started = True
        ...
        if N[1] >= 1:  # use the else case for ODMR!!!!!!!!!
            ...
            # the buffer comes here..
            # the last instruction with Inst.END_LOOP... return the control to start[0] (the first instruction)
            pbchannels = 0 ^ b_channels_only
            # status = pb_inst_pbonly(pbchannels, Inst.END_LOOP, start[0], t_buffer[1]) if t_buffer[1] >= 10 else 0
            # NOT 0 for else case since the END_LOOP inst is required...
            status = pb_inst_pbonly(pbchannels, Inst.END_LOOP, start[0], t_buffer[1]) if t_buffer[1] >= 10 \
                else pb_inst_pbonly(pbchannels, Inst.END_LOOP, start[0], 10*ns)
            self.errorCatcher(status)
            # print(f"Buffer: {pbchannels}, {Inst.END_LOOP}, {start[0]}, {t_buffer[1]}") if t_buffer[1] >= 10 else print(f"Buffer: {pbchannels}, {Inst.END_LOOP}, {start[0]}, {10*ns} ({t_buffer[1]})")
            ...
            self.errorCatcher(start[2])
            # print(f"{pbchannels}, {Inst.END_LOOP}, {start[0]}, {t_exposure[1]}")

        # final camera trigger pulse
        # if self.sequence == 'esr_seq':
        pbchannels = concfg.camera ^ concfg.laser ^ b_channels_only
        pb_inst_pbonly(pbchannels, Inst.CONTINUE, 0, t_cam_response)
        # print(f"End:, {pbchannels}, {Inst.CONTINUE}, {0}, {t_cam_response}")
        
        pbchannels = concfg.laser ^ b_channels_only
        pb_inst_pbonly(pbchannels, Inst.STOP, 0, 14*ns) 
        # print(f"{pbchannels}, {Inst.STOP}, {0}, {14*ns}")
        
        # else:
        #     pb_inst_pbonly(concfg.camera ^ concfg.laser, Inst.CONTINUE, 0, t_cam_response)
        #     print(f"End:, {concfg.camera ^ concfg.laser}, {Inst.CONTINUE}, {0}, {t_cam_response}")
        #     pb_inst_pbonly(concfg.laser, Inst.STOP, 0, 14*ns) 
        #     print(f"{concfg.laser}, {Inst.STOP}, {0}, {14*ns}")
        #---------------------------------------------DONE-------------------------------------------
    ...
    def run_sequence_for_camera_level_trigger_many_bdc(self, instructionList, t_exposure, t_align, t_seq_total, N_total):
        # t_seq_total should be a list: t_seq_total = [t_seq_tot_sig, t_seq_tot_ref]
        # print(instructionList)
        # instructionList               => len(instructionList)=2; PB instructions for 'signal' and 'reference' sequences
        # instructionList[i]            => i=0: signal, i=1: reference
    ...
    # TODO: change name to run_sequence_for_camera_level_trigger_many_ac()
    def run_sequence_for_camera_level_trigger_many_bac(self, instructionList, t_exposure, t_align,
                                                      t_seq_total, N_total, Nsamples):
        # with level trigger
        # t_seq_total should be a list: t_seq_total = [t_seq_tot_sig, t_seq_tot_ref]
        # print(instructionList)
        # instructionList               => len(instructionList)=2; PB instructions for 'signal' and 'reference' sequences
        ...
        # part 1: aligning the propeller: here concfg.start_trig signals the start of `new` rotating field sequence
        b_channels = concfg.bx ^ concfg.by ^ concfg.bz ^ concfg.start_trig      # concfg.start_trig goes to DAQ to trigger the ac output
        b_channels_only = concfg.bx ^ concfg.by ^ concfg.bz
        
        status = pb_inst_pbonly(b_channels, Inst.CONTINUE, 0, t_align);
        self.errorCatcher(status)
        # print(f"\x1b[38;2;250;100;0mSignal\x1b[0m: {b_channels}, {Inst.CONTINUE}, {0}, {t_align}")
        # consider the inductance of the coil and adjust the timescale
        status = pb_inst_pbonly(b_channels_only, Inst.CONTINUE, 0, 45*ms);
        self.errorCatcher(status)
        # print(f"{b_channels_only}, {Inst.CONTINUE}, {0}, {5*ms}")
        start = []  # store the 'start' points of the sequence: 1st instruction and loop start point

        # part 2: camera exposure start edge and subsequent delay
        # do we need laser here?
        pbchannels = concfg.camera ^ concfg.laser ^ b_channels_only#^ concfg.MW
        start.append(pb_inst_pbonly(pbchannels, Inst.LOOP, Nsamples, t_cam_response))
        self.errorCatcher(start[0])
        ...
        if N[0] >= 1:  # use the else case for ODMR!!!!!!!!!
            ...
            for i in range(0, len(instruction) - 1):
                ...
                if started:
                    # middle instructions...
                    pbchannels = instruction[i][0] ^ b_channels_only ^ concfg.camera
                    status = pb_inst_pbonly(pbchannels, instruction[i][1], instruction[i][2], instruction[i][3]);
                    self.errorCatcher(status)
                    # print(f"{pbchannels} ({instruction[i][0]}), {instruction[i][1]}, {instruction[i][2]}, {instruction[i][3]}")
                else:
                    # first instruction.. with Inst.LOOP and repetitions N_trigger(_signal)[0]
                    # start[1]
                    pbchannels = instruction[0][0] ^ b_channels_only ^ concfg.camera
                    start.append(pb_inst_pbonly(pbchannels, Inst.LOOP, N[0], instruction[0][3]));
                    self.errorCatcher(start[1])
                    # print(f"{pbchannels} ({instruction[i][0]}), {Inst.LOOP}, {N[0]}, {instruction[0][3]}")
                    started = True
            ...
            # the last instruction.. with Inst.END_LOOP and start point (start[2])...
            pbchannels = instruction[i + 1][0] ^ b_channels_only ^ concfg.camera
            status = pb_inst_pbonly(pbchannels, Inst.END_LOOP, start[1], instruction[i + 1][3]);
            self.errorCatcher(status)
            # print(f"Last: {pbchannels}, {Inst.END_LOOP}, {start[1]}, {instruction[i + 1][3]}")

            # the buffer comes here..
            pbchannels = 0 ^ b_channels_only ^ concfg.camera
            # status = pb_inst_pbonly(pbchannels, Inst.CONTINUE, 0, t_buffer[0]) if t_buffer[0] >= 10 else 0
            # NOT 0 for else case since the CONTINUE inst is required...
            status = pb_inst_pbonly(pbchannels, Inst.CONTINUE, 0, t_buffer[0]) if t_buffer[0] >= 10 \
                else pb_inst_pbonly(pbchannels, Inst.CONTINUE, 0, 10*ns)
            ...
            pbchannels = concfg.laser ^ concfg.MW ^ b_channels_only ^ concfg.camera
            # start[1]
            # there is no buffer in this case
            # Exposure and laser happens for the same time
            start.append(pb_inst_pbonly(pbchannels, Inst.CONTINUE, 0, t_exposure[0]));
        ...
        # TODO: automate the gap between frames
        t_gap = 8*ms
        pbchannels = 0 ^ b_channels_only#^ concfg.laser
        status = pb_inst_pbonly(pbchannels, Inst.CONTINUE, 0, t_gap)
        # print(f"Gap: {pbchannels}, {Inst.CONTINUE}, {0}, {t_gap}")
        self.errorCatcher(status)
        ...
        if N[1] >= 1:  # use the else case for ODMR!!!!!!!!!
            ...
            # status = pb_inst_pbonly(pbchannels, Inst.CONTINUE, start[0], t_buffer[1]) if t_buffer[1] >= 10 else 0
            # NOT 0 for else case since the CONTINUE inst is required...
            status = pb_inst_pbonly(pbchannels, Inst.CONTINUE, start[0], t_buffer[1]) if t_buffer[1] >= 10 \
                else pb_inst_pbonly(pbchannels, Inst.CONTINUE, start[0], 10*ns)
            self.errorCatcher(status)
            # print(f"Buffer: {pbchannels}, {Inst.CONTINUE}, {start[0]}, {t_buffer[1]}") if t_buffer[1] >= 10 else print(f"Buffer: {pbchannels}, {Inst.CONTINUE}, {start[0]}, {10*ns} ({t_buffer[1]})")

        else:  # check this out for normal ODMR!!!!!!
            pbchannels = concfg.laser ^ b_channels_only ^ concfg.camera
            # start[2]
            start.append(pb_inst_pbonly(pbchannels, Inst.CONTINUE, 0, t_exposure[1]));
            self.errorCatcher(start[2])
            # print(f"{pbchannels}, {Inst.CONTINUE}, {0}, {t_exposure[1]}")
        ...
        # the field (b_channels_only) can be kept ON for the whole measurement and might save 5 ms each run
        pb_inst_pbonly(0, Inst.CONTINUE, 0, 14*ns)
        # print(f"End:, {0}, {Inst.CONTINUE}, {0}, {14*ns}")
        pb_inst_pbonly(0, Inst.STOP, 0, 10*ns)
        # print(f"{0}, {Inst.STOP}, {0}, {10*ns}")
    ...
    def run_sequence_for_camera_sync_trigger_many_bac(self, instructionList, t_exposure, t_align, t_seq_total, N_total, Nsamples):
        # with sync-readout trigger
        # t_seq_total should be a list: t_seq_total = [t_seq_tot_sig, t_seq_tot_ref]
        # print(instructionList)
        # instructionList               => len(instructionList)=2; PB instructions for 'signal' and 'reference' sequences
    ...
    def focus_adjustment_sequence(self, instr, t_exposure, Nsamples, focus_time: float,
                                  t_align=2*8*ms,):
        """PB sequence during the focus adjustment. Designs and loads the sequence but does not start the sequence.
        ...
        ----------
            ...
            t_align : float
                ...
                (for 'cam_level', take t_align as 2*8ms -> 2*exposure + 2*gap)
            Nsamples : int
        ...
        start = []
        if not instr == 'cam_levelm':
            b_channels = concfg.bx ^ concfg.by ^ concfg.bz ^ concfg.start_trig      # open/close all PB channels for rotating field alignment
            # b_channels_only = concfg.bx ^ concfg.by ^ concfg.bz
            start.append(pb_inst_pbonly(b_channels ^ concfg.laser, Inst.LOOP, self.n_repeat, t_align))
            self.errorCatcher(start[0])
            status = pb_inst_pbonly(concfg.laser, Inst.END_LOOP, 0, 5*ms + Nsamples*t_exposure)
            self.errorCatcher(status)
        else: # for 'cam_levelm' we don't need have any magnetic field control
            b_channels = concfg.bx ^ concfg.by ^ concfg.bz
            pb_inst_pbonly(concfg.laser, Inst.LOOP, self.n_repeat, t_align)
            pb_inst_pbonly(concfg.laser, Inst.END_LOOP, 0, 5*ms + Nsamples*t_exposure)
        status = pb_stop_programming();    self.errorCatcher(status)
