"""Sample for standard capture control with DCAM.
Sample script for setting basic DCAM properties to a camera.
"""
#%%
__date__ = '2023-12-06'
...
import time, PBcontrol_v2 as pbctrl, connectionConfig as concfg
import signal
...
def show_live_captured_images(dcamcon: Dcamcon, instructionList):
    """Show live images.
    ...
    if frameinterval is not False:
        ...
        timeout_ms = int((exposuretime + frameinterval) * 1000.0) + 500
    else:
        # set timeout waiting for a frame to arrive to exposure time + 1 second
        timeout_ms = int(exposuretime * 1000.0) + 1000
    ...
    global signaled_sigint
    wait_event_time=[]
    i=0; Nsamples = 6; Nscanpts = 30
    data_raw = np.zeros((Nscanpts, Nsamples, 2048, 2048), dtype='uint16')
    # while cv_window_status >= 0:
    for i_scanpt in range(0,Nscanpts*2):
        if signaled_sigint:
            break
        print('i_scanpt: ', i_scanpt)
        frames = np.zeros((Nsamples, 2048, 2048), dtype='uint16')

        if i_scanpt%2==0:
            print('Starting PB sequence...')
            t1 = time.perf_counter()
            res = dcamcon.wait_capevent_frameready(timeout_ms)
            wait_event_time.append(time.perf_counter()-t1)
        ...
        if res is not True:
            ...
            if timeout_happened == 1:
                ...
                if triggersource == DCAMPROP.TRIGGERSOURCE.EXTERNAL:
                    ...
                    print(' Check your <timeout_ms> calculation in the code.', end='')
        ...
        for j in range(0, Nsamples):
            print(j,end='')
            if j > 0:
                result = dcamcon.wait_capevent_frameready(timeout_ms)
            # wait_capevent_frameready() succeeded
            frame = dcamcon.get_lastframedata()
            if frame is not False:
                print(' frame elo...')
                frames[j, :, :] = frame
            else:
                print('No frame...')
            show_framedata(dcamcon.device_title, frame)
    ...
    dcamcon.releasebuffer()
    return wait_event_time, frame, frames, data_raw
...
if __name__ == '__main__':
    ...
    if dcamcon_init():
        ...
        if dcamcon is not None:
            ...
            if (not signaled_sigint and
                res):
                all_vars = show_live_captured_images(dcamcon, instructionList)
