"""Sample for framebundle control with DCAM.
Sample script for setting properties related framebundle to a camera.
"""
#%%
__date__ = '2023-12-06'
...
def setup_properties(dcamcon: Dcamcon):
    ...
    attr_hsize = dcamcon.dcam.prop_getattr(DCAM_IDPROP.SUBARRAYHSIZE)
    attr_vsize = dcamcon.dcam.prop_getattr(DCAM_IDPROP.SUBARRAYVSIZE)

    hsize = attr_hsize.valuemin
    vsize = attr_vsize.valuemin
    ...
    attr_bundlenum = dcamcon.dcam.prop_getattr(DCAM_IDPROP.FRAMEBUNDLE_NUMBER)
    bundlenum = attr_bundlenum.valuemax
